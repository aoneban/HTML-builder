...
const sourceDir_2 = path.join(__dirname, 'assets');
const destinationDir_2 = path.join(__dirname, 'project-dist', 'assets');
...
function copyDirectoryRec(source, destination, callback) {
  ...
  copyDirectoryRec
(sourceDir_2, destinationDir_2, (err) => {
    if (err) {
      console.error(`Error: ${err}`);
    } else {
      console.log('Success');
...
fs.mkdir(destinationDir, { recursive: true }, (err) => {
    ...
    fs.readFile(path.join(__dirname, 'styles','header.css'), 'utf8', (err, data1) => {
        if (err) {
          console.error(`Error: ${err}`);
          return;
        }
      
        fs.readFile(path.join(__dirname, 'styles','main.css'), 'utf8', (err, data2) => {
          if (err) {
            console.error(`Error: ${err}`);
            return;
          }
    
          fs.readFile(path.join(__dirname, 'styles','footer.css'), 'utf8', (err, data3) => {
            if (err) {
              console.error(`Error: ${err}`);
              return;
            }
            ...
            fs.writeFile(path.join(__dirname, 'project-dist','style.css'), combinedData, 'utf8', (err) => {
              if (err) {
                console.error(`Error: ${err}`);
              } else {
                console.log(`Success`);
